	...
	switch(type)
	{
		...
		case MQTT_PKT_PUBLISH:														//接收的Publish消息
			...
			if(result == 0)
			{
				...
				//解析数据包
				json = cJSON_Parse(req_payload);				//获取所接收到的整个json包
				if(!json)	UsartPrintf(USART_DEBUG,"ERROR before [%s]\n",cJSON_GetErrorPtr());
				else
				{
					
					if((json_value = cJSON_GetObjectItem(json,"LED_SW"))!=NULL)
					{
						if(json_value->valueint)		//LED_SW所带值>0且为整形
						{
							LED_SW=1;
							LED0=1;			//灯亮
						}
					...
					if((json_value = cJSON_GetObjectItem(json,"BEEP_SW"))!=NULL)
					{
						if(json_value->valueint)		//BEEP_SW所带值>0且为整形
						{
							dht11_flag=1;							//打开蜂鸣器
							dht11_flag_free=0;				//控制10s
						}
						else
						{
							dht11_flag=0;							//关闭蜂鸣器
